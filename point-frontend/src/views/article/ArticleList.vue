    ...
    <div v-for="a in articles" :key="a.id" class="article-item" @click="$router.push(`/articles/${a.id}`)">
      <div class="d-flex">
        <div class="flex-grow-1">
          <div class="mb-1" style="font-size: 16px; font-weight: 500; color: var(--paper-text); line-height: 1.4">
            {{ a.title }}
          </div>
        ...
        <div
          ...
          style="width: 80px; height: 60px; border-radius: 6px; overflow: hidden"
        >
          <img :src="a.cover" style="width: 100%; height: 100%; object-fit: cover" />
        </div>
      </div>
    </div>
    <div v-if="articles.length === 0 && !loading" class="text-center py-16" style="color: var(--paper-text2)">
      暂无文章
    </div>
...
async function loadItems(reset = false) {
  ...
  try {
    ...
    if (data.code === 0) {
      const payload = (data.data || {}) as PageResult<Article>
