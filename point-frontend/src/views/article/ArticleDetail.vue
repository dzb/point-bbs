  ...
  <div v-if="article">
    <div class="detail-main">
      ...
      <article>
        <header class="mb-6">
          ...
          <div class="d-flex align-center text-caption" style="color: var(--paper-text2)">
            <router-link
              ...
              style="color: var(--paper-text2); text-decoration: none"
            >
              <UserAvatar :src="article.user?.avatar" :name="article.user?.nickname" :size="22" class="mr-2" />
              {{ article.user?.nickname
              }}<span style="font-size: 11px; margin-left: 4px; color: var(--paper-text2)">@{{ article.user?.username || article.userId }}</span>
            </router-link>
        ...
        <div class="article-body" @click="onContentClick" v-html="rendered" />
      ...
      <section>
        <div class="d-flex align-center mb-4">
          <h3 style="font-weight: 500; color: var(--paper-text); margin: 0; font-size: 17px">
            评论 {{ comments.length }}
          </h3>
        </div>
        <div v-if="auth.isLoggedIn" class="mb-4">
          <MentionTextarea
            v-model="commentText"
            placeholder="写下你的想法... @用户名 可提及用户"
            rows="2"
            class="mb-2"
          />
          <div class="d-flex justify-end">
            <v-btn variant="text" :loading="submitting" style="color: var(--paper-text2)" @click="postComment">
              发布
            </v-btn>
          </div>
        </div>
          ...
          <div class="d-flex">
            ...
            <div class="flex-grow-1">
              <div class="d-flex align-center mb-1">
                <span style="font-size: 14px; color: var(--paper-text); font-weight: 500">{{ c.user?.nickname
                }}<span style="font-size: 11px; color: var(--paper-text2); font-weight: 400">
                  @{{ c.user?.username || c.user?.id }}</span></span>
...
async function loadComments(reset = false) {
  ...
  const id = route.params.id as string
  try {
    const { data } = await client.get(`/articles/${id}/comments`, {
