<template>
  <div v-if="profile" class="mx-auto" style="max-width:680px">
    ...
    <div v-if="tab==='topics'">
      <div v-for="t in topics" :key="t.id" class="py-3" style="border-bottom:1px solid var(--paper-border);cursor:pointer"
        @click="$router.push(`/topics/${t.id}`)">
        <div style="font-size:15px;color:var(--paper-text);font-weight:500;line-height:1.4">{{ t.title || '无标题' }}</div>
        <div style="font-size:12px;color:var(--paper-text2);margin-top:4px">{{ t.commentCount||0 }} 评论 · {{ fmt(t.createTime) }}</div>
      </div>
