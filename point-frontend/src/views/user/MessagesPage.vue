  ...
  <div class="detail-main">
    ...
    <div v-for="m in filtered" :key="m.id" class="msg-card" :class="{ unread: m.status === 0 }">
      <div class="d-flex align-center mb-2">
        <v-icon :color="m.status === 0 ? 'var(--paper-accent)' : ''" size="16" class="mr-2 flex-shrink-0">
          {{
            iconForType(m.type)
          }}
        </v-icon>
        ...
        <span style="font-size: 12px; color: var(--paper-text2); margin-left: auto; flex-shrink: 0" class="ml-2">{{
          time(m.createTime)
        }}</span>
      </div>
      <div
      ...
      <div v-if="replyId === m.id" class="mt-2">
        ...
        <div class="d-flex justify-end" style="gap: 8px">
          <v-btn variant="text" size="x-small" style="text-transform: none; letter-spacing: 0" @click.stop="replyId = 0">
            取消
          </v-btn>
          <v-btn
            variant="flat"
    ...
    <div v-if="filtered.length === 0 && !loading" class="text-center py-16" style="color: var(--paper-text2)">
      暂无通知
    </div>
...
const filtered = computed(() => {
  ...
  return messages.value.filter((m) => m.type === t)
})

onMounted(() => {
  // auth.user is populated asynchronously by App.vue — wait for it before
  // firing the messages request (avoiding /users/undefined/messages)
  if (auth.user?.id) loadItems()
  else waitForUser()
})
...
async function markAllRead() {
  try {
    // Backend read-all endpoint — the old {ids: []} call was a no-op
    await client.post(`/users/${auth.user?.id}/messages/read-all`)
...
function parseExtra(m: Message): { type: string; id: string; parentType?: string; parentId?: string } | null {
  if (!m.extraData) return null
  // New format: JSON {"type":"topic","id":142}; legacy: "topic:142" / "comment:108:topic:142"
  if (m.extraData.startsWith('{')) {
    try {
      const o = JSON.parse(m.extraData)
      return {
        type: String(o.type ?? ''),
        id: String(o.id ?? ''),
        parentType: o.parentType ? String(o.parentType) : undefined,
...
function canOpen(m: Message): boolean {
...
function entityLabel(m: Message): string {
  const e = parseExtra(m)
  if (!e) return ''
  const t = e.parentType || e.type
...
function actionText(m: Message): string {
...
function canReply(m: Message): boolean {
  const e = parseExtra(m)
  if (!e) return false
  const t = e.parentType || e.type
...
function toggleReply(m: Message) {
  replyId.value = replyId.value === m.id ? 0 : m.id
  replyText.value = ''
}

async function doReply(m: Message) {
