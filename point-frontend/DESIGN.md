...
```
│ 侧栏 240 │  主内容区       32px │ 32px  公告区 366px │
           │  max-width 680px     │                    │
```

- 主内容区左右 padding：32px（基准），1200px→20px，900px→16px
- 公告区卡片：padding 12px，间距 12px，标题 16px，正文 14px
- 侧栏 <1200px 自动收缩为图标，公告区 <1100px 隐藏
- 所有页面（首页/详情/创作/收藏/消息/用户）统一此布局
