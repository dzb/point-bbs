...
public class UploadRoutes {
    ...
    public static RouteGroup routes() {
        return RouteGroup.of("/api",
            Route.post("/upload", UploadHandler.class),
            Route.get("/attachment/download/{id}", DownloadAttachmentHandler.class)
        );
    }

    public static final class UploadHandler implements RouteHandler {
        private final UploadService svc;

        public UploadHandler(UploadService svc) {
            this.svc = svc;
        }
    ...
    public static final class DownloadAttachmentHandler implements RouteHandler {
        private final UploadService svc;
        private final Database db;

        public DownloadAttachmentHandler(UploadService svc, Database db) {
            this.svc = svc;
            this.db = db;
        }
        ...
        public void handle(HttpContext ctx) throws Exception {
            ...
            boolean dispositionInline = (UploadService.isImage(type) && !svg);
            ctx.setHeader("Content-Type", type);
            ctx.setHeader("Content-Disposition", (dispositionInline ? "inline" : "attachment") + "; filename=\"" + att.getFileName() + "\"");
